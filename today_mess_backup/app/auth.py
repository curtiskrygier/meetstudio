...
from app.config import CLIENT_ID, SECRET_KEY
...
def encrypt_token(token: str) -> str:
    """Encrypts a plain-text token for 'at-rest' storage in memory/db."""
    if not token: return ""
    return _fernet.encrypt(token.encode()).decode()

def decrypt_token(encrypted_token: str) -> str:
    """Decrypts an encrypted token for use in API calls."""
    if not encrypted_token: return ""
    try:
        return _fernet.decrypt(encrypted_token.encode()).decode()
    except Exception as e:
        logger.error(f"[auth] Decryption failed: {e}")
        return ""

# -- Producer API auth (STAGE_API_KEY) -----------------------------------------

_STAGE_API_KEY = os.environ.get("STAGE_API_KEY", "")
_IS_PROD = os.environ.get("K_SERVICE") or os.environ.get("GAE_SERVICE")

if not _STAGE_API_KEY:
    if _IS_PROD:
        logger.error("STAGE_API_KEY not set in production! Aborting to prevent open access.")
        raise RuntimeError("STAGE_API_KEY must be set in production environments.")
    else:
        logger.warning("STAGE_API_KEY not set — producer API endpoints are OPEN to the world (Local Dev Mode)")
