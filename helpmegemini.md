...
- `CLOUD_PROJECT_NUMBER` — the GCP project number (`649226456677`)
- `CLIENT_ID` — the OAuth client ID (`649226456677-lf3vsget4m8c7cati2k0eki33n0l19lt.apps.googleusercontent.com`)
...
RUN echo "CLIENT_ID=649226456677-lf3vsget4m8c7cati2k0eki33n0l19lt.apps.googleusercontent.com" > .env && \
    echo "CLOUD_PROJECT_NUMBER=649226456677" >> .env
RUN npm run build
...
```
...
Despite multiple attempts, the built JS bundle being served from Cloud Run does NOT contain `649226456677`. The bundle always contains an empty string for the project number:

```js
// In the minified bundle, nc is the cloudProjectNumber variable
const nc = ""   // <-- should be "649226456677"
```

Confirmed by:
```bash
curl -s https://meet-live-concierge-649226456677.us-central1.run.app/assets/index-Bfzv048q.js \
  | python3 -c "import sys; print('649226456677' in sys.stdin.read())"
# Output: False
```
...
1. `--set-build-env-vars="CLIENT_ID=...,CLOUD_PROJECT_NUMBER=649226456677"` on the gcloud deploy command — values do not reach Vite
...
4. `RUN echo "CLIENT_ID=..." > .env && echo "CLOUD_PROJECT_NUMBER=649226456677" >> .env` in Dockerfile — layer appears to be cached by Cloud Build, bundle unchanged
...
gcloud run deploy meet-live-concierge \
  --source . \
  --build-arg CLIENT_ID=649226456677-... \
  --build-arg CLOUD_PROJECT_NUMBER=649226456677 \
  ...
```
...
- name: 'gcr.io/cloud-builders/docker'
  args:
    ...
    - --no-cache
    - --build-arg
    - CLIENT_ID=649226456677-lf3vsget4m8c7cati2k0eki33n0l19lt.apps.googleusercontent.com
    - --build-arg
...
```bash
CLIENT_ID=649226456677-... CLOUD_PROJECT_NUMBER=649226456677 npm run build
```
...
| Project Number | `649226456677` |
| Cloud Run service | `meet-live-concierge` |
| Region | `us-central1` |
| Service URL | `https://meet-live-concierge-649226456677.us-central1.run.app` |
| Client ID | `649226456677-lf3vsget4m8c7cati2k0eki33n0l19lt.apps.googleusercontent.com` |
